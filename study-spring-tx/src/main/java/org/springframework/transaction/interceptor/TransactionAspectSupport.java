 ...
 *
 * <p>This enables the underlying Spring tx infrastructure to be used easily
 * to implement an aspect for any aspect system.
 *
 * <p>Subclasses are responsible for calling methods in this class in the correct order.
 *
 * <p>If no tx name has been specified in the {@link TransactionAttribute},
 * the exposed name will be the {@code fully-qualified class name + "." + method name}
 * (by default).
 *
 * <p>Uses the <b>Strategy</b> design pattern. A {@link PlatformTransactionManager} or
 * {@link ReactiveTransactionManager} implementation will perform the actual tx
 * management, and a {@link TransactionAttributeSource} (e.g. annotation-based) is used
 * for determining tx definitions for a particular class or method.
 *
 * <p>A tx aspect is serializable if its {@code PlatformTransactionManager}
...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
    ...
    /**
     * Key to use to store the default tx manager.
     */
    ...
    private static final ThreadLocal<TransactionInfo> transactionInfoHolder =
            new NamedThreadLocal<>("Current aspect-driven tx");
     ...
     * <p>A TransactionInfo will be returned even if no tx was created.
     * The {@code TransactionInfo.hasTransaction()} method can be used to query this.
     * <p>To find out about specific tx characteristics, consider using
    ...
    /**
     * Return the tx status of the current method invocation.
     * Mainly intended for code that wants to set the current tx
     * rollback-only but not throw an application exception.
     * @throws NoTransactionException if the tx info cannot be found,
     * because the method was invoked outside an AOP invocation context
     */
    public static TransactionStatus currentTransactionStatus() throws NoTransactionException {
        TransactionInfo info = currentTransactionInfo();
        if (info == null || info.transactionStatus == null) {
            throw new NoTransactionException("No tx aspect-managed TransactionStatus in scope");
        }
    ...
    /**
     * Specify the name of the default tx manager bean.
     * This can either point to a traditional {@link PlatformTransactionManager} or a
     * {@link ReactiveTransactionManager} for reactive tx management.
     */
    ...
    /**
     * Return the name of the default tx manager bean.
     */
    @Nullable
    ...
    /**
     * Specify the <em>default</em> tx manager to use to drive transactions.
     * This can either be a traditional {@link PlatformTransactionManager} or a
     * {@link ReactiveTransactionManager} for reactive tx management.
     * <p>The default tx manager will be used if a <em>qualifier</em>
     * has not been declared for a given tx or if an explicit name for the
     * default tx manager bean has not been specified.
    ...
    /**
     * Return the default tx manager, or {@code null} if unknown.
     * This can either be a traditional {@link PlatformTransactionManager} or a
     * {@link ReactiveTransactionManager} for reactive tx management.
     */
    @Nullable
    ...
    /**
     * Set properties with method names as keys and tx attribute
    ...
    /**
     * Set multiple tx attribute sources which are used to find tx
    ...
    /**
     * Set the tx attribute source which is used to find tx
    ...
    /**
     * Return the tx attribute source.
     */
    @Nullable
        ...
        if (getTransactionAttributeSource() == null) {
            throw new IllegalStateException(
                    "Either 'transactionAttributeSource' or 'transactionAttributes' is required: " +
                            "If there are no transactional methods, then don't use a tx aspect.");
        }
    }
    ...
    protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
        ...
        // If the tx attribute is null, the method is non-transactional.
            ...
            ReactiveTransactionSupport txSupport = this.transactionSupportCache.computeIfAbsent(method, key -> {
                if (KotlinDetector.isKotlinType(method.getDeclaringClass()) && KotlinDelegate.isSuspend(method)) {
                    throw new TransactionUsageException(
                            "Unsupported annotated tx on suspending function detected: " + method +
                                    ". Use TransactionalOperator.transactional extensions instead.");
                }
                ReactiveAdapter adapter = this.reactiveAdapterRegistry.getAdapter(method.getReturnType());
                if (adapter == null) {
                    throw new IllegalStateException("Cannot apply reactive tx to non-reactive return type: " +
        ...
        if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
            // Standard tx demarcation with getTransaction and commit/rollback calls.
    ...
    /**
     * Determine the specific tx manager to use for the given tx.
     */
    @Nullable
    ...
    private PlatformTransactionManager asPlatformTransactionManager(@Nullable Object transactionManager) {
        if (transactionManager == null || transactionManager instanceof PlatformTransactionManager) {
            ...
            throw new IllegalStateException(
                    "Specified tx manager is not a PlatformTransactionManager: " + transactionManager);
        }
    }
    ...
    /**
     * Create a tx if necessary based on the given TransactionAttribute.
     * <p>Allows callers to perform custom TransactionAttribute lookups through
     * the TransactionAttributeSource.
     * @param txAttr the TransactionAttribute (may be {@code null})
     * @param joinpointIdentification the fully qualified method name
     * (used for monitoring and logging purposes)
     * @return a TransactionInfo object, whether or not a tx was created.
     * The {@code hasTransaction()} method on TransactionInfo can be used to
     * tell if there was a tx created.
    ...
    protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
        ...
        // If no name specified, apply method identification as tx name.
            ...
            if (tm != null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Skipping transactional joinpoint [" + joinpointIdentification +
                            "] because no tx manager has been configured");
                }
            }
        }
     ...
     * @param joinpointIdentification the fully qualified method name
     * (used for monitoring and logging purposes)
     * @param status the TransactionStatus for the current tx
    ...
    protected TransactionInfo prepareTransactionInfo(@Nullable PlatformTransactionManager tm,
        ...
        if (txAttr != null) {
            // We need a tx for this method...
            if (logger.isTraceEnabled()) {
                logger.trace("Getting tx for [" + txInfo.getJoinpointIdentification() + "]");
            }
            // The tx manager will flag an error if an incompatible tx already exists.
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("No need to create tx for [" + joinpointIdentification +
                        "]: This method is not transactional.");
            }
        }

        // We always bind the TransactionInfo to the thread, even if we didn't create
        // a new tx here. This guarantees that the TransactionInfo stack
        // will be managed correctly even if no tx was created by this aspect.
        txInfo.bindToThread();
        return txInfo;
    }
     ...
     * Do nothing if we didn't create a tx.
     * @param txInfo information about the current tx
     */
    protected void commitTransactionAfterReturning(@Nullable TransactionInfo txInfo) {
        if (txInfo != null && txInfo.getTransactionStatus() != null) {
            if (logger.isTraceEnabled()) {
                logger.trace("Completing tx for [" + txInfo.getJoinpointIdentification() + "]");
            }
    ...
    /**
     * Handle a throwable, completing the tx.
     ...
     * @param txInfo information about the current tx (may be {@code null})
     */
    ...
    /**
     * Opaque object used to hold tx information. Subclasses
    ...
    protected static final class TransactionInfo {
        ...
        /**
         * Return whether a tx was created by this aspect,
        ...
        private void bindToThread() {
            ...
            // for restoration after this tx is complete.
            this.oldTransactionInfo = transactionInfoHolder.get();
            transactionInfoHolder.set(this);
        }

        private void restoreThreadLocalStatus() {
            // Use stack to restore old tx TransactionInfo.
        ...
        public String toString() {
            return (this.transactionAttribute != null ? this.transactionAttribute.toString() : "No tx");
        }
    }
    ...
    /**
     * Internal holder class for a Throwable in a callback tx model.
     */
    ...
    private class ReactiveTransactionSupport {
        ...
        private ReactiveTransactionInfo prepareTransactionInfo(@Nullable ReactiveTransactionManager tm,
            ...
            if (txAttr != null) {
                ...
                // to preserve the integrity of the ThreadLocal stack maintained in this class.
                if (logger.isTraceEnabled()) {
                    logger.trace("Don't need to create tx for [" + joinpointIdentification +
    ...
    /**
     * Opaque object used to hold tx information for reactive methods.
     */
